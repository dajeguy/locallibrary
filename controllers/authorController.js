...
exports.author_list = function(req, res, next) {
  ...
  Author.find()
    ...
    .exec(function (err, list_authors) {
      if (err) { 
        debug('get author list error:' + err);
        return next(err); 
    }
...
exports.author_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { 
        debug('get author detail error:' + err);
        return next(err); 
        } // Error in API usage.
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            author.save(function (err) {
                if (err) { 
                    debug('create author post error:' + err);
                    return next(err); 
                }
...
exports.author_delete_get = function(req, res, next) {
    async.parallel({
	   ...
	   if(err) { 
        debug('delete author get error:' + err);
        return next(err);
    }
	   if(results.author == null) { // No results.
...
exports.author_delete_post = function(req, res, next) {
    ...
    function(err, results){
	if(err) { 
        debug('delete author post error:' + err);
        return next(err); 
    }
	// Success
	if(results.authors_books.length > 0){
	  ...
	  Author.findByIdAndRemove(req.body.authorid, function deleteAuthor(err){
	      if(err) { 
            debug('author delete remove error:' + err);
            return next(err);
        }
	      res.redirect('/catalog/authors')
	  })
